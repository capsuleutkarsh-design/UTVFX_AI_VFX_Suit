...
class TrackerWorker(BaseWorker):
    ...
    def run_task(self):
        ...
        if image_dir and os.path.basename(image_dir) == "Video Plate":
            potential_jpg_dir = image_dir + " JPG"
            if os.path.isdir(potential_jpg_dir):
                self.log_message.emit(self.node_id, "Intelligently routing to pre-rendered Media Plate JPEGs for tracking...")
                image_dir = potential_jpg_dir
