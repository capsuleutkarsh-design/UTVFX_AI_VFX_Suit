...
class SuperMatteWorker(BaseWorker):
    ...
    def run_task(self):
        ...
        refiner_mode = self.params.get("refiner_mode", "ViTMatte (ONNX/TensorRT)")
        ...
        try:
            if refiner_mode == "MEMatte (Local)":
                import sys
                from models.MEMatte.mematte_loader import build_mematte_refiner
                model = build_mematte_refiner(device=device)
                model.eval()
                use_mematte = True
                ...
                if os.path.exists(onnx_path):
                    ...
                    processor = VitMatteImageProcessor.from_pretrained("hustvl/vitmatte-base-composition-1k")
                    model = VitMatteForImageMatting.from_pretrained("hustvl/vitmatte-base-composition-1k", use_safetensors=True).to(device)
                    model.eval()
            else:
