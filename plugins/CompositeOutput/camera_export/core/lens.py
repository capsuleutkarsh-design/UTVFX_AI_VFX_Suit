...
def exr_backend(recheck=False):
    ...
    if backend is None:
        # Contour VFX ships OpenImageIO, which writes and reads 32-bit float EXR exactly.
        try:
...
def _write_exr_file(path, arr, backend):
    ...
    if backend == "oiio":
        ...
        spec = oiio.ImageSpec(width, height, channels, oiio.TypeFloat)
        spec.channelnames = tuple(names)
        out = oiio.ImageOutput.create(str(path))
        if out is None or not out.open(str(path), spec):
            raise IOError("cannot write %s: %s" % (path, oiio.geterror()))
        out.write_image(np.ascontiguousarray(arr))
        out.close()
        return
...
def read_exr(path):
    ...
    if backend == "oiio":
        ...
        pix = buf.get_pixels(oiio.TypeFloat)
        return pix if pix.ndim == 3 else pix[:, :, None]
