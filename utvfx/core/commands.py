...
class AddNodeCommand(QUndoCommand):
    ...
    def redo(self):
        ...
        self.node.is_disabled = self.node_data.get("disabled", False)
...
class DisconnectCommand(QUndoCommand):
    def __init__(self, scene, connection, description="Disconnect Nodes"):
        ...
        self.port1 = connection.port1
        self.port2 = connection.port2

    def redo(self):
        if self.connection in self.scene.connections:
            self.scene.connections.remove(self.connection)
        self.port1.remove_connection(self.connection)
        if self.port2:
            self.port2.remove_connection(self.connection)
        self.scene.removeItem(self.connection)

    def undo(self):
        self.scene.addItem(self.connection)
        self.port1.add_connection(self.connection)
        if self.port2:
            self.port2.add_connection(self.connection)
