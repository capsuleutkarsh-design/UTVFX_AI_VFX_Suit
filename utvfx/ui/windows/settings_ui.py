...
class SettingsDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.out_edit = QLineEdit(self.settings_manager.get("default_output_dir") or "")
        self.out_edit.setPlaceholderText("Inside each project folder")
        self.out_edit.setReadOnly(True)
        btn_out_browse = QPushButton("Browse...")
        btn_out_browse.setIcon(icons.icon("folder"))
        btn_out_browse.clicked.connect(lambda: self.browse_folder(self.out_edit, "default_output_dir"))
        self._add_row(grid, 0, "Default output", self.out_edit, btn_out_browse)

        self.cache_edit = QLineEdit(self.settings_manager.workspace_dir)
        self.cache_edit.setReadOnly(True)
        btn_cache_browse = QPushButton("Browse...")
        btn_cache_browse.setIcon(icons.icon("folder"))
        btn_cache_browse.clicked.connect(lambda: self.browse_folder(self.cache_edit, "workspace_dir"))
        self._add_row(grid, 1, "Workspace (renders and cache)", self.cache_edit, btn_cache_browse)
