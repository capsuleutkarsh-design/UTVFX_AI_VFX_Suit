...
- [ ] Frame contract: timeline *position* inside the app, plate *frame number* only in file names (C7)
- [ ] Inputs resolved from the actual wire, with typed ports (H2)
...
**Stability:**
- [ ] Undo by node id (C5). Keyframes survive reload (C6). Dot nodes save (H1)
- [ ] Engine: cache marker (H3), Stop/cancel (H4), Freeze/Bypass (H5), In/Out (H7), hash at start (H8), Add Node menus (H6)
- [ ] Projects: unsaved-changes prompt, atomic save, cache kept on Save As (M1, M2). Crash logging (M4)
- [ ] AI bridge: off-UI-thread shutdown, timeouts, visible errors (M3)

**Security and install:**
- [ ] Pin BiRefNet revision and load locally only (H9). `weights_only=True`, ZIP importer limited to `models/` (H10). Download hashes (M6)
