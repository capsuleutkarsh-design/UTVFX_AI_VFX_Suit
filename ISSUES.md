...
| H19 | Unified Output | Gamma 2.2 applied to alpha and depth. "16-bit float" writes 32-bit. sRGB values in EXR. Mattes land in Y, not A | 3 | | ☑ Colour layers from the full-quality master into ACEScg / ACES2065-1 / linear Rec.709 EXR (half or full float as chosen) or display PNG through the ACES view; mattes into A and depth into Z with no colour or gamma; plate frame numbers and In/Out; optional combined RGBA + depth.Z EXR (premultiplied) and core/edge mattes; shot_reads.nk (data layers raw) and a JSON sidecar |
...
| M11 | Licensing | CorridorKey, VideoMaMa, GVM, MatAnyone, Depth V2 Base/Large are non-commercial. No THIRD_PARTY_NOTICES | before release | ◐ THIRD_PARTY_NOTICES.md written (every model, program and package checked) and shipped with the build and installer; MatAnyone code is gone. Still open, and only needed before sharing or selling: remove, replace or license CorridorKey (its terms also forbid bundling without Corridor Digital's agreement), GVM, VideoMaMa/SVD and Depth V2 Base/Large |
